...
**The simplest way to use the whole project is the web app (Foundry).** From the
project root:
...
npm start
```
Then open **http://localhost:3001** in your browser. Pick a dataset and a mode
(Sequential / OpenMP / MPI / Hybrid) and click **Run**. Press `Ctrl+C` to stop.

> First time only (one-time setup): `cd web\client; npm install; npm run build; cd ..\server; npm install`

**Prefer the terminal?** Build once, then run a version:

```powershell
.\build.ps1                                                              # build (Windows)
build\seq_pipeline.exe data\agnews\agnews_10k.txt results\out.txt 5      # Sequential
build\omp_pipeline.exe data\agnews\agnews_10k.txt results\out.txt 5 8    # OpenMP, 8 threads
```

(MPI & Hybrid run in WSL — see [`command.txt`](command.txt) for the simple
step-by-step, or the sections below for full detail.)
...
- [x] All 4 stages independently verified on the dataset (`docs/report.md` §11)
...
## 🖥️ Interactive Web UI — Foundry

A professional, web-based console named **Foundry** lives in **`web/`** (React +
Vite + Recharts frontend, Node + Express backend). It **runs the real pipeline
live** and visualizes the benchmarks for every version. The four versions are
presented as **Phase 1–4** (Sequential, OpenMP, MPI, Hybrid).

- **Run Console** — pick a dataset and a mode (**Sequential · OpenMP · MPI ·
  Hybrid**), set `min_words` / processes / threads, and execute. Sequential &
  OpenMP run the Windows `.exe`; **MPI & Hybrid run inside WSL via `mpirun`**.
  Shows live counters, a **speedup vs sequential**, a per-stage timing chart, the
  exact command, the output SHA-256, and a **byte-identical-to-sequential** badge.
- **Benchmarks** — interactive OpenMP / MPI / Hybrid speedup & efficiency charts
  built from `results/benchmark_*.csv`.
- **About** — the pipeline, the design philosophy, and the end-to-end story.

```powershell
# prerequisites: .\build.ps1 (seq+omp) and, for MPI/Hybrid, ./build.sh in WSL
...
├── web/                          # interactive web UI (runs the pipeline live)
│   ├── client/                   #   React + Vite + Recharts frontend
│   ├── server/                   #   Node + Express backend (runs exe / WSL mpirun)
│   └── README.md                 #   web UI setup & API reference
└── docs/
    ├── report.md                 # THE complete project report (all phases)
    ├── report.pdf                # the report as a professional PDF
    ├── graph_openmp_speedup.png      #   performance graphs embedded in report
    ├── graph_openmp_efficiency.png
    ├── graph_mpi_speedup.png
    ├── graph_hybrid_speedup.png
    ├── ui_header.png                     #   Foundry web UI screenshots
    ├── ui_config/results/timings/output.png
    ├── ui_about_stages.png / ui_about_end.png
